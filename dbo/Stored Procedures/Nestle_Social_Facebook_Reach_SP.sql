...
create procedure [dbo].[Nestle_Social_Facebook_Reach_SP]
AS
begin

	WITH Facebook_Reach_Table AS
	(
	--Adding required columns from Reach Table
	SELECT
	  date_start AS 'Campaign Start Date',
		date_stop AS 'Campaign Stop Date',
		account_name AS 'Account',
		adset_name AS 'Adset Name',
		campaign_name AS 'Campaign',
		reach AS 'Reach',
		frequency AS 'Frequency'
	  FROM [dbo].[Nestle_Social_Facebook_Reach]
	  where (reach != 0 or frequency != 0)
	),
		--Data Manipulations / Mapping brand names from nestle naming conventions master file (Campaign mapping tab)
	Facebook_Reach_Brand_Table
	AS(
  		SELECT *,
			CASE
			--------------------------------BEVERAGES--------------------------------------------------------------
	...
	--setting up column language based on  pattern matching
			CASE
				...
				ELSE 'Unknown'
				END AS [Language]
	FROM Facebook_Reach_Table
	),
	---- Corp Check loop 2 

	Facebook_Reach_Brand_Filter_Table 
	AS
	(
	...
	SELECT *, 
			   CASE
				...
				FROM Facebook_Reach_Brand_Table
				)

--Adding Missing Brand names from the default brand list table
--Adding Missing Brand names from the default brand list table
SELECT *
FROM Facebook_Reach_Brand_Filter_Table AS a
