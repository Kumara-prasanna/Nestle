...
create procedure [dbo].[Nestle_Social_Executive_Summary_SP] 
as
begin

	--Combining required columns from facebook & twitter data table

	WITH Facebook_Twitter_Table
	AS (
		(
	...
	(
		SELECT [timestamp] AS 'Date',
			...
			'unknown' AS 'Gender',
			'unknown' AS 'Age',
			--NULL AS 'Creative Thumbnail URL',
			--NULL AS 'Creative Image URL',
			--NULL AS 'Creative Title',
			--NULL AS 'Creative Body',
			--tweet_account AS 'Twitter Account',
			--tweet AS 'tweet',
			costs AS 'Spend',
			impressions AS 'Impressions',
			clicks AS 'Clicks',
			...
			--engagements AS 'Tweets Engagement',
			--NULL AS 'Video Plays',
			video_total_views AS 'Video Views',
			video_views_25 AS 'Video Views - 25%',
			video_views_50 AS 'Video Views - 50%',
			video_views_75 AS 'Video Views - 75%',
			video_views_100 AS 'Video Views - 100%'
			--NULL AS 'FilterName'
		FROM [dbo].[Nestle_Social_Twitter_Ads]
		where (clicks != 0 or impressions !=0 )
		)
	...
	Facebook_Twitter_Brand_Filter_Table 
	AS
	(
	...
	SELECT *, 
			   CASE
				...
				FROM Facebook_Twitter_Brand_Table
				)

--Adding Missing Brand names from the default brand list table

SELECT *
FROM Facebook_Twitter_Brand_Filter_Table AS a
