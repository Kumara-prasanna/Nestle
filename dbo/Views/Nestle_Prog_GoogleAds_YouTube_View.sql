...
CREATE VIEW [dbo].[Nestle_Prog_GoogleAds_YouTube_View] 
...
SELECT	[account] AS 'Account',
		...
		[cost] AS 'Cost',
		[views] AS [Video Views],
		CAST(video_played_to_25 AS bigint) AS [Video Views - 25%],
        video_played_to_50 AS  [Video Views - 50%],
        CAST(video_played_to_75 AS bigint) AS [Video Views - 75%],
        CAST(video_played_to_100 AS bigint)  AS [Video Views - 100%],
		[day]AS 'Date',
		[device] AS 'Device',
...
--setting up column language based on  pattern matching
       ...
       CASE 
		 ...
		 WHEN (Campaign LIKE '%/_NB%' Escape '/' )   OR LOWER(Campaign) LIKE '%non brand%' OR LOWER(Labels) LIKE '%non-brand%'  THEN 'Non-Brand'
	...
	CASE
		WHEN [Network With Search Partners] = 'Cross-network' THEN 'Cross-Network'
		WHEN [Network With Search Partners] = 'Display Network' THEN 'Display Network'
		WHEN [Network With Search Partners] = 'Google search' THEN 'Google Search'
		WHEN [Network With Search Partners] = 'YouTube Videos' THEN 'YouTube Videos'
...
SELECT *, 
	       CASE
			...
			WHEN ((LOWER(Campaign) LIKE '%master%' OR LOWER([Client Customer Name])  LIKE '%purina%') AND (LOWER(Campaign) LIKE '%master%' OR LOWER([Client Customer Name])  LIKE '%purina%')) AND (FilterName NOT LIKE '%Purina%') THEN 'Purina-Purina Master Brand'
			ELSE FilterName
