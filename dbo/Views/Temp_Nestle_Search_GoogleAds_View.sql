...
CREATE VIEW [dbo].[Temp_Nestle_Search_GoogleAds_View] 
...
SELECT	[account] AS 'Account',
		...
		NULL AS 'Match Type',
		[search_impr_share] AS 'Search Impression Share',
		'Nestle_Google_Ads' as [table]
FROM [Nestle_Google_Ads]
WHERE ([views] >0 ) OR (LOWER(network) NOT LIKE '%youtube%')
...
AS (
	SELECT *,
		...
		--------------------------------BEVERAGES--------------------------------------------------------------
			...
			WHEN LOWER(Campaign) LIKE '%swsw%' OR LOWER([Client Customer Name]) LIKE '%swsw%'  THEN 'Infant Nutrition-SWSW'
			WHEN (LOWER(Campaign) LIKE '%baby%' OR LOWER([Client Customer Name]) LIKE '%baby%') AND (LOWER(Campaign) NOT LIKE '%cetaphil%' OR LOWER([Client Customer Name]) NOT LIKE '%cetaphil%' ) THEN 'Infant-Nestle Baby'
			...
			WHEN LOWER(Campaign) LIKE '%mpk%' OR LOWER([Client Customer Name]) LIKE '%mpk%'  THEN 'Purina-MPK'
