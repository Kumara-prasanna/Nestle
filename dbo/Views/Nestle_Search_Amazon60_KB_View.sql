...
CREATE VIEW [dbo].[Nestle_Search_Amazon60_KB_View]
AS
...
SELECT
	...
	NULL AS 'Targeting Data Source', --since its missing in 60 days view
	'N/A' AS 'ASIN',
...
FROM [dbo].[Netsle_AMS_S_Brands_60Days]
...
FROM [dbo].[Nestle_AMS_Products_KW_60Days]
where (clicks > 0 or impressions > 0)
...
FROM [dbo].[Nestle_AMS_Products_ProdAds_60Days] as a  
	JOIN (SELECT distinct [ASIN]
      ,[Products] from [dbo].[Nestle_Search_Amazon_ASIN_Mapping]
 )b ON a.[ASIN] = b.[ASIN]
where (clicks > 0 or impressions > 0) 
and campaignname not in (select campaignname from [dbo].[Nestle_AMS_Products_KW_60Days])
)
,
