...
CREATE VIEW [dbo].[Nestle_Prog_AmazonDSP_View]

AS

--Combining required columns from facebook & twitter data table

WITH Amazon_Display_Table
AS (
	(
	SELECT	a.dt_updated AS 'Date',
		a.Line_item AS 'Line Item',
		a.Advertiser AS 'Account',
		a.Creative AS 'Creative',
		creative_type,
		a.Total_cost AS 'Cost',
		a.Impressions AS 'Impressions',
		a.clickthroughs AS 'Clicks',
		a.Mashup_CC_click_clicks AS 'Mashup Clicks',
		a.mashup_click_to_page_clicks AS 'Mashup Clicks CPA',
		a.DPV AS 'Detailed Page Views',
		a.DPVR AS 'Detailed Page View Rate',
		a.ATL AS 'Add To List',
		a.Units_sold AS 'Units Sold',
		a.Sales_CAD AS 'Sales',
		a.newtobrand_purchases AS 'New To Brand Purchases',
		a.newtobrand_product_sales_cad AS 'New To Brand Product Sales',
		a.total_newtobrand_units_sold AS 'New To Brand Units Sold',
		p.campaignname AS 'Campaign',
CASE WHEN p.CampaignName like '2020_%' THEN '2020'
	 WHEN p.CampaignName like '2019_%' THEN '2019'
	 ELSE 'X-Year'
END AS 'Campaign_Year'
from	[dbo].[Nestle_Prog_AmazonDSP] a
		LEFT JOIN [dbo].[Nestle_All_Prisma_CampaignNames] p on a.creative = p.placementname
		where (a.clickthroughs > 0 or a.impressions > 0)
	)
...
AS (
	SELECT *,
		...
		--------------------------------BEVERAGES--------------------------------------------------------------
			...
			WHEN LOWER(Campaign) LIKE '%ONE%' OR LOWER([Account]) LIKE '%ONE%'  THEN 'Purina-ONE'
...
--setting up column language based on  pattern matching
		...
		WHEN LOWER(Creative) LIKE '%double%' THEN 'Double Click Bid Manager'
		...
		-- Ad Type Category - KB
,CASE
WHEN creative_type IN ('Rodeo HTML - MobileSDK','Shazam HTML - AAP','Thorium','Thorium Mobile','Thorium Mobile Web','ThoriumUniversalDSP') THEN 'Banner'
WHEN creative_type = 'Video Template with Automated Transcoding' THEN 'Video'
WHEN (creative_type IS NULL AND lower(creative) like '%banner%')  THEN 'Banner'
WHEN (creative_type IS NULL AND lower(creative) like '%video%')  THEN 'Video'
	
ELSE 'Others'
END AS [Ad_Type_Category]


	FROM Amazon_Display_Table
	)
...
Amazon_Display_Brand_Table_Filter_Table 
AS
(
...
SELECT *, 
	       CASE
			...
			WHEN ((LOWER(Campaign) LIKE '%master%' OR LOWER([Account])  LIKE '%purina%') AND (LOWER(Campaign) LIKE '%master%' OR LOWER([Account])  LIKE '%purina%')) AND (FilterName NOT LIKE '%Purina%') THEN 'Purina-Purina Master Brand'
			ELSE FilterName
			END AS FilterNameUpdated
			FROM Amazon_Display_Brand_Table
			)
...
SELECT *
FROM 
Amazon_Display_Brand_Table_Filter_Table  AS a
