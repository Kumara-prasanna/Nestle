CREATE TABLE [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_tracking] (
    ...
    CONSTRAINT [PK_DataSync.CD_Facebook_Ads_Reach_ToDate_dss_tracking] PRIMARY KEY CLUSTERED ([campaign_id] ASC, [account_id] ASC)
);


GO
CREATE NONCLUSTERED INDEX [tombstone_index]
    ON [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_tracking]([sync_row_is_tombstone] ASC, [local_update_peer_timestamp] ASC)
    INCLUDE([last_change_datetime]);


GO
CREATE NONCLUSTERED INDEX [local_update_peer_timestamp_index]
    ON [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_tracking]([local_update_peer_timestamp] ASC, [campaign_id] ASC, [account_id] ASC);
