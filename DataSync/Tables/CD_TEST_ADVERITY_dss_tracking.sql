CREATE TABLE [DataSync].[CD_TEST_ADVERITY_dss_tracking] (
    ...
    CONSTRAINT [PK_DataSync.CD_TEST_ADVERITY_dss_tracking] PRIMARY KEY CLUSTERED ([campaign_id] ASC, [ad_id] ASC, [adset_id] ASC, [age_ag] ASC, [account_id] ASC, [date_start] ASC, [gender_ag] ASC)
);


GO
CREATE NONCLUSTERED INDEX [tombstone_index]
    ON [DataSync].[CD_TEST_ADVERITY_dss_tracking]([sync_row_is_tombstone] ASC, [local_update_peer_timestamp] ASC)
    INCLUDE([last_change_datetime]);


GO
CREATE NONCLUSTERED INDEX [local_update_peer_timestamp_index]
    ON [DataSync].[CD_TEST_ADVERITY_dss_tracking]([local_update_peer_timestamp] ASC, [campaign_id] ASC, [ad_id] ASC, [adset_id] ASC, [age_ag] ASC, [account_id] ASC, [date_start] ASC, [gender_ag] ASC);
