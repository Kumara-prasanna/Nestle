CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Age_Gender_dss_bulkdelete_86428056-fea2-4062-8e10-5f42d853c559]
...
DELETE [dbo].[CD_Facebook_Ads_Age_Gender] 
OUTPUT DELETED.[campaign_id], DELETED.[ad_id], DELETED.[adset_id], DELETED.[age_ag], DELETED.[account_id], DELETED.[date_start], DELETED.[gender_ag] INTO @changed FROM [dbo].[CD_Facebook_Ads_Age_Gender] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[CD_Facebook_Ads_Age_Gender_dss_tracking] t ON p.[campaign_id] = t.[campaign_id] AND p.[ad_id] = t.[ad_id] AND p.[adset_id] = t.[adset_id] AND p.[age_ag] = t.[age_ag] AND p.[account_id] = t.[account_id] AND p.[date_start] = t.[date_start] AND p.[gender_ag] = t.[gender_ag]) as changes ON changes.[campaign_id] = base.[campaign_id] AND changes.[ad_id] = base.[ad_id] AND changes.[adset_id] = base.[adset_id] AND changes.[age_ag] = base.[age_ag] AND changes.[account_id] = base.[account_id] AND changes.[date_start] = base.[date_start] AND changes.[gender_ag] = base.[gender_ag] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
