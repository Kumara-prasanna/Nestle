CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Reach_Weekly_dss_selectrow_86428056-fea2-4062-8e10-5f42d853c559]
	@P_5 Date,
	@P_6 BigInt,
	@P_10 BigInt,
	@P_11 Date,
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[CD_Facebook_Ads_Reach_Weekly] WHERE [date_stop] = @P_5 AND [campaign_id] = @P_6 AND [account_id] = @P_10 AND [date_start] = @P_11) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[CD_Facebook_Ads_Reach_Weekly_dss_tracking] WHERE [date_stop] = @P_5 AND [campaign_id] = @P_6 AND [account_id] = @P_10 AND [date_start] = @P_11) [side] ON [base].[date_stop] = [side].[date_stop] AND [base].[campaign_id] = [side].[campaign_id] AND [base].[account_id] = [side].[account_id] AND [base].[date_start] = [side].[date_start]
END