CREATE PROCEDURE [DataSync].[CD_TEST_ADVERITY_dss_delete_86428056-fea2-4062-8e10-5f42d853c559]
	@P_5 BigInt,
	@P_9 BigInt,
	@P_13 BigInt,
	@P_17 NVarChar(200),
...
SET @sync_row_count = 0; DELETE [dbo].[CD_TEST_ADVERITY] FROM [dbo].[CD_TEST_ADVERITY] [base] LEFT JOIN [DataSync].[CD_TEST_ADVERITY_dss_tracking] [side] ON [base].[campaign_id] = [side].[campaign_id] AND [base].[ad_id] = [side].[ad_id] AND [base].[adset_id] = [side].[adset_id] AND [base].[age_ag] = [side].[age_ag] AND [base].[account_id] = [side].[account_id] AND [base].[date_start] = [side].[date_start] AND [base].[gender_ag] = [side].[gender_ag] WHERE ((CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL AND [side].[local_update_peer_timestamp] > @marker_local_update_peer_timestamp 
