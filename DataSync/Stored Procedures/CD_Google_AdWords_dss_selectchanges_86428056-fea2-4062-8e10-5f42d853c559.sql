CREATE PROCEDURE [DataSync].[CD_Google_AdWords_dss_selectchanges_86428056-fea2-4062-8e10-5f42d853c559]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [dbo].[CD_Google_AdWords] [base] FULL OUTER JOIN [DataSync].[CD_Google_AdWords_dss_tracking] [side] ON [base].[client_customer_id] = [side].[client_customer_id] AND [base].[network_w_search_partners] = [side].[network_w_search_partners] AND [base].[campaign_id] = [side].[campaign_id] AND [base].[day] = [side].[day] AND [base].[deviceseg] = [side].[deviceseg]
 WHERE 
