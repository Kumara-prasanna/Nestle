CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_bulkinsert_86428056-fea2-4062-8e10-5f42d853c559]
...
(SELECT p.*, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_tracking] t ON p.[campaign_id] = t.[campaign_id] AND p.[account_id] = t.[account_id]) AS changes ON changes.[campaign_id] = base.[campaign_id] AND changes.[account_id] = base.[account_id]
WHEN NOT MATCHED BY TARGET AND changes.local_update_peer_timestamp <= @sync_min_timestamp OR changes.local_update_peer_timestamp IS NULL THEN
INSERT ([dt_created], [dt_updated], [dt_filename], [impressions], [campaign_id], [reach], [inline_link_clicks], [campaign_name], [account_id], [date_start], [account_name], [frequency], [spend], [campaign_stop_time], [campaign_start_time]) VALUES (changes.[dt_created], changes.[dt_updated], changes.[dt_filename], changes.[impressions], changes.[campaign_id], changes.[reach], changes.[inline_link_clicks], changes.[campaign_name], changes.[account_id], changes.[date_start], changes.[account_name], changes.[frequency], changes.[spend], changes.[campaign_stop_time], changes.[campaign_start_time])
