CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_selectchanges_86428056-fea2-4062-8e10-5f42d853c559]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [dbo].[CD_Facebook_Ads_Reach_ToDate] [base] FULL OUTER JOIN [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_tracking] [side] ON [base].[campaign_id] = [side].[campaign_id] AND [base].[account_id] = [side].[account_id]
 WHERE 
