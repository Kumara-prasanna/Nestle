CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Publisher_Platform_dss_selectchanges_86428056-fea2-4062-8e10-5f42d853c559]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [dbo].[CD_Facebook_Ads_Publisher_Platform] [base] FULL OUTER JOIN [DataSync].[CD_Facebook_Ads_Publisher_Platform_dss_tracking] [side] ON [base].[campaign_id] = [side].[campaign_id] AND [base].[ad_id] = [side].[ad_id] AND [base].[adset_id] = [side].[adset_id] AND [base].[publisher_platform_pp] = [side].[publisher_platform_pp] AND [base].[platform_position_pp] = [side].[platform_position_pp] AND [base].[account_id] = [side].[account_id] AND [base].[date_start] = [side].[date_start]
 WHERE 
