CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Age_Gender_dss_selectrow_86428056-fea2-4062-8e10-5f42d853c559]
	@P_5 BigInt,
	@P_9 BigInt,
	@P_13 BigInt,
	@P_17 NVarChar(200),
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[CD_Facebook_Ads_Age_Gender] WHERE [campaign_id] = @P_5 AND [ad_id] = @P_9 AND [adset_id] = @P_13 AND [age_ag] = @P_17 AND [account_id] = @P_30 AND [date_start] = @P_32 AND [gender_ag] = @P_33) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[CD_Facebook_Ads_Age_Gender_dss_tracking] WHERE [campaign_id] = @P_5 AND [ad_id] = @P_9 AND [adset_id] = @P_13 AND [age_ag] = @P_17 AND [account_id] = @P_30 AND [date_start] = @P_32 AND [gender_ag] = @P_33) [side] ON [base].[campaign_id] = [side].[campaign_id] AND [base].[ad_id] = [side].[ad_id] AND [base].[adset_id] = [side].[adset_id] AND [base].[age_ag] = [side].[age_ag] AND [base].[account_id] = [side].[account_id] AND [base].[date_start] = [side].[date_start] AND [base].[gender_ag] = [side].[gender_ag]
END