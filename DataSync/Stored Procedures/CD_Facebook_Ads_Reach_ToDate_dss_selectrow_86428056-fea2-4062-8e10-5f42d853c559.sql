CREATE PROCEDURE [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_selectrow_86428056-fea2-4062-8e10-5f42d853c559]
	@P_5 BigInt,
	@P_9 BigInt,
...
as [account_id], 
[base].[dt_created], [base].[dt_updated], [base].[dt_filename], [base].[impressions], [base].[reach], [base].[inline_link_clicks], [base].[campaign_name], [base].[date_start], [base].[account_name], [base].[frequency], [base].[spend], [base].[campaign_stop_time], [base].[campaign_start_time], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[CD_Facebook_Ads_Reach_ToDate] WHERE [campaign_id] = @P_5 AND [account_id] = @P_9) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[CD_Facebook_Ads_Reach_ToDate_dss_tracking] WHERE [campaign_id] = @P_5 AND [account_id] = @P_9) [side] ON [base].[campaign_id] = [side].[campaign_id] AND [base].[account_id] = [side].[account_id]
END